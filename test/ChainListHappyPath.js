...
contract('ChainList', function(accounts){
  ...
  var articleDescription2 = "Description for article 12";
  ...
  it("should be initialized with empty values", function() {
    return ChainList.deployed().then(function(instance) {
      ...
      assert.equal(data.toNumber(), 0, "number or articles must be 0");
      return chainListInstance.getArticlesForSale();
    }).then(function(data){
      assert.equal(data.length,0, "there should not be any articles for sales");
    });
  });
  ...
  it("should let us sell a first article", function() {
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._name, articleName1, "event article name 1 must be " + articleName1);
      assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei( articlePrice1,"ether"), "event article price1 must be " + web3.toWei( articlePrice1,"ether"));
      return chainListInstance.getNumberOfArticles();
    }).then(function(data){
...
// sell a second articles
  it("should let us sell a second article", function() {
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._seller, seller, "event seller must be " + seller);
      assert.equal(receipt.logs[0].args._name, articleName2, "event article name 2 must be " + articleName2);
      assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei( articlePrice2,"ether"), "event article price2 must be " + web3.toWei( articlePrice2,"ether"));
      return chainListInstance.getNumberOfArticles();
    }).then(function(data){
  ...
  it("should buy an article", function(){
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei( articlePrice1,"ether"), "event article price 1 must be " + web3.toWei( articlePrice1,"ether"));
      ...
      assert.equal(data.length,1,"articles left for sale must be 1");
